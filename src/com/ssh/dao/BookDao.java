...
//	数据图书类型查询总页数
	public int getclassifynum(int booktype){
		...
		try{
			...
			String hql="from Book as b where deleteFlag=0 and booktype.tid=? ";
...
//	根据图书id查询图书
	public Book getbook(int id){
		...
		try{
			session=sessionFactory.openSession(); //得到session对象
			String hql="from Book where deleteFlag=0 and bid=? ";
			Query query=session.createQuery(hql);
...
//	根据图书id获取图书类型
	public String getbooktype(int id){
		...
		try{
			session=sessionFactory.openSession(); //得到session对象
			String hql="select b.booktype.btype from Book as b where b.deleteFlag=0 and bid=? ";
			Query query=session.createQuery(hql);
			query.setParameter(0, id);
...
//	根据图书类型编号查询图书类型
	...
	public List classifypricedesc(int booktype,int pageNo){
		...
		try{
			session=sessionFactory.openSession(); //得到session对象
			String hql="from Book as b where deleteFlag=0 and booktype.tid=? order by price desc"
					+ "";
			Query query=session.createQuery(hql);
			query.setParameter(0, booktype);
			query.setFirstResult((pageNo-1)*9);
	...
	//图书分类价格升序
		public List classifypriceasc(int booktype,int pageNo){
			...
			try{
				session=sessionFactory.openSession(); //得到session对象
				String hql="from Book as b where deleteFlag=0 and booktype.tid=? order by price asc"
						+ "";
				Query query=session.createQuery(hql);
				query.setParameter(0, booktype);
				query.setFirstResult((pageNo-1)*9);
	...
	//图书分类销量降序
		public List classifysalesAmountsort(int booktype,int pageNo){
			...
			try{
				session=sessionFactory.openSession(); //得到session对象
				String hql="from Book as b where deleteFlag=0 and booktype.tid=? order by salesAmount desc"
						+ "";
				Query query=session.createQuery(hql);
				query.setParameter(0, booktype);
				query.setFirstResult((pageNo-1)*9);
		...
		public List classifysalesAmountasc(int booktype,int pageNo){
			...
			try{
				session=sessionFactory.openSession(); //得到session对象
				String hql="from Book as b where deleteFlag=0 and booktype.tid=? order by salesAmount asc"
						+ "";
				Query query=session.createQuery(hql);
				query.setParameter(0, booktype);
				query.setFirstResult((pageNo-1)*9);
		...
		public List classifypricerange(int booktype,int pageNo,float min,float max){
			...
			try{
				session=sessionFactory.openSession(); //得到session对象
				String hql="from Book as b where deleteFlag=0 and booktype.tid=? and price>=? and price<=?"
						+ "";
				Query query=session.createQuery(hql);
				query.setParameter(0, booktype);
