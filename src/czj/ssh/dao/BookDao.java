...
//			list = sqlQuery.addEntity(Book.class).list();
	...
	public List<Book> fuzzyLookup(String keyword,String type){
		...
		try {
			session = sessionFactory.openSession();
			String hql = "from Book b where deleteFlag=0 and b." + type + " like '%" + keyword + "%'";
	...
	//根据图书类型查找图书
	public List<Book> queryBookByType(int tid){
		List<Book> list = new ArrayList();
		Session session = null;
		try {
			session = sessionFactory.openSession();
			String hql = "from Book as b where b.booktype.tid = ?";
			Query query = session.createQuery(hql);
