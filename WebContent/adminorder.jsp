...
<script type="text/javascript">
//** iframe自动适应页面 **//
//输入您希望根据页面高度自动调整高度的iframe的名称的列表
//用逗号把每个iframe的ID分隔. 例如: ["myframe1", "myframe2"]，能够只有一个窗体，则不用逗号。
//定义iframe的ID
var iframeids=["test"]
//假如用户的浏览器不支持iframe是否将iframe隐藏 yes 表示隐藏，no表示不隐藏
var iframehide="yes"
...
if (dyniframe.contentDocument && dyniframe.contentDocument.body.offsetHeight) //假如用户的浏览器是NetScape
dyniframe.height = dyniframe.contentDocument.body.offsetHeight; 
else if (dyniframe.Document && dyniframe.Document.body.scrollHeight) //假如用户的浏览器是IE
